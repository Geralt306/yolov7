...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin, TSCODE_Detect))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // 2
        elif m in [Detect, TSCODE_Detect, IDetect, IAuxDetect, IBin, IKeypoint]:
